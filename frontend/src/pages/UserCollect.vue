<template>
  <n-layout embedded class="full_page">
    <BackgroundImage/>
    <n-layout position="absolute" class="collect_content" :native-scrollbar="false">
          ...
          <n-list-item v-for="(item, index) in dataList" :item="item" :index="index" :key="item.id">
            <router-link :to="generateUrl(item)" #="{ navigate, href }" custom>
              <n-space justify="space-between" align="center" :wrap="false">
                ...
                <div>
                  <n-a v-if="showJumpLink" tag="div" :underline="false" :href="href" @click="navigate">
                    <span>{{ str_time(item.time) }}</span>
                  </n-a>
                  <span v-else>{{ str_time(item.time) }}</span>
                  <n-button type="error" style="margin-left: 8px" @click="removeCollect(item.id)">取消收藏</n-button>
                </div>
              </n-space>
            </router-link>
          </n-list-item>
...
function removeCollect(post_id) {
...
function generateUrl(item) {
  ...
  return "/post/detail/" + item.id + "?keyword=" +
      (item.tags === "" ? item.content.substr(0, 4) : tag) + "&from=collect"
}
